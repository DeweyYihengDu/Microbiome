...
conc.info <- read_excel(here('poster/files','Suppl_Table_1.xlsx')) %>% 
  filter(prestwick_ID %in% overlap.abx)
...
ggsave(g, filename = here('poster/figures','EDFig3a.pdf'),
...
if (dir.exists(here('poster/source_data'))){
  df.plot %>% 
    write_tsv(here('poster/source_data', 'EDFig3a.tsv'))
  df.table %>% 
    write_tsv(here('poster/source_data', 'EDFig3a-1.tsv'))
}
