...
CB <- read_tsv(here('poster/data','CB_community_pathogen_raw_data.tsv'))
...
pdf(here('poster/figures', 'Fig3c.pdf'),width=6,height=4) 
ggarrange(`Erythromycin_Dicumarol_community + Enterococcus faecalis_alt`,
          ncol=1, nrow=1)
dev.off()


pdf(here('poster/figures', 'EDFig10c.pdf'),width=6,height=4) 
...
if (dir.exists(here('poster/source_data'))){
  ...
  CB_combined %>% 
    ...
    write_tsv(here('poster/source_data', 'Fig3c.tsv'))
  
  CB_combined %>% 
    filter(DrugX=='Benzbromarone') %>% 
    select(DrugX, DrugXConc, ABX, ABXConc, median_normAUC, mean_cfu, sd_cfu) %>%
    write_tsv(here('poster/source_data', 'EDFig10c.tsv'))
}