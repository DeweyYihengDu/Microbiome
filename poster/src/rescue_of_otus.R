...
samples <- read_tsv(here("poster/data", "antidotes_samples.tsv"), col_types = cols(
...
counts <- read_tsv(here("poster/data", "antidotes_16S_counts.tsv.gz"), 
                   col_types = cols(
                     otu = col_character(),
...
pp <- grid.arrange(set_panel_size(p, width = unit(6, "cm"), 
                                  height = unit(2, "cm")))
ggsave(here("poster/figures", "Fig3d.pdf"), pp, height = 16, width = 8, units = "cm")

# export source data
if (dir.exists(here('poster/source_data'))){
  d %>% 
    filter(n_reduced > 0) %>% 
    write_tsv(here('poster/source_data', 'Fig3d.tsv'))
}
...
pp <- grid.arrange(set_panel_size(p, 
                                  width = unit(6, "cm"), 
                                  height = unit(2, "cm")))
ggsave(here("poster/figures", "Fig3e.pdf"), pp, height = 16, width = 8, units = "cm")

# export source data
if (dir.exists(here('poster/source_data'))){
  dd %>% 
    select(-lbl) %>% 
    write_tsv(here('poster/source_data', 'Fig3e.tsv'))
}
...
ggsave(here("poster/figures", "Fig3f.pdf"), pp, height = 16, width = 8, units = "cm")

# export source data
if (dir.exists(here('poster/source_data'))){
  dd %>% 
    select(-lbl) %>% 
    write_tsv(here('poster/source_data', 'Fig3f.tsv'))
}