...
CB <- read_tsv(here('poster/data','CB_commensals_raw_data.tsv'))
...
pdf(here('poster/figures', 'EDFig7d.pdf'),width=18,height=12) 
...
pdf(here('poster/figures', 'EDFig9a.pdf'),width=18,height=12) 
...
if (dir.exists(here('poster/source_data'))){
  ...
  CB_combined %>% 
    ...
    write_tsv(here('poster/source_data', 'EDFig7d.tsv'))
  
  CB_combined %>% 
    filter(Strain_name %in% c('B. fragilis NT', 'B. thetaiotaomicron', 
                              'P. copri', 'B. ovatus', 'B. caccae', 
                              'P. distasonis')) %>% 
    write_tsv(here('poster/source_data', 'EDFig9a.tsv'))
}
...
# load all checkerboards (CB) for commensals
CB <- read_tsv(here('poster/data','CB_pathogens_raw_data.tsv'))
...
pdf(here('poster/figures', 'EDFig9b.pdf'),width=18,height=12) 
...
if (dir.exists(here('poster/source_data'))){
  CB_combined %>% 
    write_tsv(here('poster/source_data', 'EDFig9b.tsv'))
}