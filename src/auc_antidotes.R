...
pathogens <- read_tsv(here('data','CB_pathogens_raw_data.tsv'))
...
commensals <- read_tsv(here('data','CB_commensals_raw_data.tsv'))
...
pdf(here('figures',"EDFig9c.pdf"),width=6,height=4) 
...
pdf(here('figures',"EDFig11a.pdf"),width=6,height=4) 
...
if (dir.exists(here('source_data'))){
  ...
  median_commensals %>% 
    ...
    write_tsv(here('source_data', 'EDFig9c.tsv'))
  
  median_commensals %>% 
    filter(DrugX=='Tolfenamic acid') %>% 
    write_tsv(here('source_data', 'EDFig11a.tsv'))
}