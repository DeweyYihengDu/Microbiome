...
Raw_data_animal_experiment <- read_tsv(
  here('data','Raw_data_animal_experiments.tsv'))
...
pdf(here('figures',"Fig3g.pdf"),width=8.27,height=11.69)
...
pdf(here('figures',"Fig3h.pdf"),width=8.27,height=11.69)
...
pdf(here('figures',"EDFig10g.pdf"),width=8.27,height=11.69)
...
pdf(here('figures',"EDFig10h.pdf"),width=8.27,height=11.69)
...
if (dir.exists(here('source_data'))){
  ...
  annotated_data_dicum_long %>% 
    ...
    write_tsv(here('source_data', 'Fig3g.tsv'))
  # Fig3h
  annotated_data_dicum_short %>% 
    select(Mouse_ID, Time, Ery_Conc_nmol_g, Treatment) %>% 
    write_tsv(here('source_data', 'Fig3h.tsv'))
  
  
  annotated_data_benz_long %>% 
    select(Mouse_ID, Time, copies_g_feces, Treatment, Exp_ID) %>% 
    write_tsv(here('source_data', 'EDFig10g.tsv'))
  
  annotated_data_benz_short %>% 
    select(Mouse_ID, Time, Ery_Conc_nmol_g, Treatment) %>% 
    write_tsv(here('source_data', 'EDFig10h.tsv'))
}
