...
CB <- read_tsv(here('data','CB_community_raw_data.tsv'))
...
pdf(here('figures', 'Fig3a.pdf'),width=3,height=2) 
ggarrange(Erythromycin_Dicumarol_community,
          ncol=1, nrow=1)
dev.off()

pdf(here("figures", 'EDFig10a.pdf'),width=3,height=2) 
ggarrange(Erythromycin_Benzbromarone_community,
          ncol=1, nrow=1)
dev.off()

pdf(here('figures', 'EDFig11b.pdf'),width=3,height=2) 
...
if (dir.exists(here('source_data'))){
  ...
  CB_combined %>% 
    ...
    select(-Strain) %>% 
    write_tsv(here('source_data', 'Fig3a.tsv'))
  
  CB_combined %>% 
    filter(DrugX=='Benzbromarone') %>% 
    select(-Strain) %>% 
    write_tsv(here('source_data', 'EDFig10a.tsv'))
  
  CB_combined %>% 
    filter(DrugX=='Tolfenamic Acid') %>% 
    select(-Strain) %>% 
    write_tsv(here('source_data', 'EDFig11b.tsv'))
}
