...
combined_hits <- read_tsv(here('data', "combined_hits.tsv"), 
                          col_types = cols(
...
antibiotic_annotation <- read_csv(here('files','ABX_annotation.csv'))


species_annotation <-  read_tsv(here('files', "species_annotation.tsv"))

combined_pv <- read_tsv(here('data', "combined_pv.tsv"), 
                        col_types = cols(
...
pdf(here('figures',"EDFig4a.pdf"),width=18,height=12)
...
pdf(here('figures',"EDFig4b.pdf"),width=18,height=12)
...
pdf(here('figures',"EDFig4c.pdf"),width=18,height=12)
