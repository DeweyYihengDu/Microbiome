...
df.plot <- read_tsv(here('data', 'iv_community.tsv'))
...
# save
pdf(here('figures',"Fig3b.pdf"),width=5,height=4) 
dicoumarol
dev.off()


pdf(here('figures',"EDFig10b.pdf"),width=5,height=4) 
benzbromarone
dev.off()

pdf(here('figures',"EDFig11c.pdf"),width=5,height=4) 
`tolfenamic acid`
dev.off()
...
if (dir.exists(here('source_data'))){
  ...
  df.plot %>% 
    ...
    write_tsv(here('source_data', 'Fig3b.tsv'))
  ...
  df.plot %>% 
    ...
    mutate(Replicate=paste0(Row, Column)) %>% 
    select(species, abundance, group, group_2, Replicate) %>% 
    write_tsv(here('source_data', 'EDFig10b.tsv'))
  ...
  df.plot %>% 
    ...
    mutate(Replicate=paste0(Row, Column)) %>% 
    select(species, abundance, group, group_2, Replicate) %>% 
    write_tsv(here('source_data', 'EDFig11c.tsv'))
}
