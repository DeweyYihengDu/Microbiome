...
finalAUCs <- read_tsv(here('data', "finalAUCs.tsv"))
...
ggsave(here('figures', 'EDFig7b.pdf'),width=8, height=6, units='in')

# export source data
if (dir.exists(here("source_data"))) {
  write_tsv(final_mean_z_scores, file = here('source_data', 'EDFig7b.tsv'))
}
...
write_tsv(hitlist_export, file = here("files","hitlist.tsv"))
