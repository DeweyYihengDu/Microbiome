...
chembl <- read_tsv(here("data/chembl_assays.tsv"), col_types=cols(
  name = col_character(),
  ATC = col_character(),
...
compounds <- read_tsv(here("data/compounds.txt"))

eucast <- read_tsv(here("data/median_eucast.tsv"))

abundance <- read_tsv(here("data/most_abundant_bugs_full.tsv")) %>% 
  select(-NCBI_id) %>% 
  mutate(specI = stri_sub(mOTU, -5, -2))

taxon_map <- read_tsv(here("data/prok-refdb-v11.0.0_specI-v2_clusters-v1.map"), 
...
species_class <- read_tsv(here("data/species_class.tsv"), col_types = "cccc")
...
lm_ann_ <- read_tsv(here("data/species_with_counts-LM_Disease association.tsv"))
...
our_abx <- read_xlsx(here("data/MICs.xlsx")) %>% 
...
our_mic <-  read_xlsx(here("data/MICs.xlsx"), sheet = "MICs")
...
ggsave(here("figures", "EDFig3c.pdf"), width = 10, height = 5)

# export source data
if (dir.exists(here('source_data'))){
  combined_species___ %>% 
    select(1:5) %>% 
    write_tsv(here('source_data', 'EDFig3c.tsv'))
}
...
ggsave(here("figures/Fig1b.pdf"), width = 5, height = 2)
...
if (dir.exists(here('source_data'))){
  dd %>% 
    ...
    select(1:7) %>% 
    write_tsv(here('source_data', 'Fig1b.tsv'))
}
...
ggsave(here("figures", "EDFig3b.pdf"), height = 4, width = 6)

# export source data
if (dir.exists(here('source_data'))){
  mics %>% 
    write_tsv(here('source_data', 'EDFig3b.tsv'))
}
